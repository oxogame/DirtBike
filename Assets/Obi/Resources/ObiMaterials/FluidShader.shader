...
Shader "Obi/Fluid/FluidShading"
{
	...
	SubShader
	{
		...
		Pass
		{
            ...
            sampler2D _MainTex; // rgb = color, a = thickness
            float _Smoothness;
            float _Metalness;
            float _Transparency;
            ...
            {
                float4 volume = tex2D(_MainTex,uv);
		    ...
		    fout frag (v2f i)
		    {
                ...
                half3 absorption = half3(1,1,1);
                    ...
                    float atten = 1;
                        ...
                        // ambient:
                        half3 ambient = SampleSphereAmbient(eyeNormal,eyePos);
                    ...
                    // Calculates refraction and absorption (using beer's law).
                        ...
                        fixed3 refraction = tex2D(_CameraOpaqueTexture, i.uv + eyeNormal.xy * volume.a * _RefractionCoeff) * absorption;
